	...
	$.fn.provinceSelect = function () {
		...
		for (var i = 0; i < provinceAndCity.length; i++) {
			...
			for (var j = 0; j < arrProvince.length; j++) {
				...
				oOption.value = province;
				oOption.text = province;			
				oProvinceSelect.append(oOption);
			}
		}
	}

	$.fn.citySelect = function (city) {
		//添加城市选项
		var oProvinceSelect = $(this);
		for (var i = 0; i < provinceAndCity.length; i++) {
			var arrProvince = provinceAndCity[i].pac;
			for (var j = 0; j < arrProvince.length; j++) {
				...
				if (city == province) {
					var arrCitySelect = oProvinceSelect.parents("div.mainBody").find("select.citySelect").eq(0);

					//清空旧城市数据
					arrCitySelect.html('<option value="">请选择</option>');

					//添加新城市数据
					for (var k = 0; k < arrCity.length; k++) {
						var cityName = arrCity[k].name;
						var oOption = document.createElement("option");
						oOption.value = cityName;
						oOption.text = cityName;
						arrCitySelect.append(oOption);
					}
				}
			}
		}
